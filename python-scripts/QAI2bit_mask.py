...
parser.add_argument(
    ...
    type=str,
    required=True,
    help="Path to QAI file generated by FORCE.",
)
parser.add_argument(
    "-of",
    "--out_file",
...
args: Dict[str, str] = {
    key: (value[0] if isinstance(value, List) and key != "qai_bits" else value)
...
def write_raster(template_raster: gdal.Dataset, out_path: str, data: np.array) -> int:
    ...
    out_raster: Optional[gdal.Dataset] = driver.Create(
        out_path,
...
def main():
    ...
    qai_raster: gdal.Dataset = cio.read_raster(args.get("qai_file"))
    qai_array: np.ndarray = qai_raster.ReadAsArray(
        0, 0, qai_raster.RasterXSize, qai_raster.RasterYSize
    )
    ...
    qai_mask: np.ndarray = 2 + np.invert(
        np.where(qai_array != 0, 1, qai_array).astype(np.byte)
    )
