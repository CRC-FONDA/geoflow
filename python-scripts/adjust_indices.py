...
parser.add_argument(
    ...
    type=str,
    required=True,
    help="File path to output dataset.",
)
parser.add_argument(
    "-STM",
    action="store_true",
    dest="is_stm",
    help="Flag indicating if raster is spectral index or spectral temporal metric. "
...
def write_raster(
    ...
    gci: int = gdal.GCI_GrayIndex,
) -> None:
    cols, rows = data.shape
    driver: gdal.Driver = gdal.GetDriverByName(of)
    dst: gdal.Dataset = driver.Create(
        path,
        xsize=cols,
        ysize=rows,
        bands=1,
...
def scale_array(
    data: np.ndarray,
    no_scaling: bool,
    old_no_data: Optional[float],
    new_no_data: int = -9999,
) -> np.ndarray:
    if no_scaling:
    ...
    if old_no_data:
        if no_scaling:
            scaled_array = np.where(
                scaled_array == old_no_data, new_no_data, scaled_array
            )
        else:
            scaled_old_na: int = int(old_no_data * 10_000)
            scaled_array = np.where(
                scaled_array == scaled_old_na, new_no_data, scaled_array
            )
    return scaled_array
...
def generate_band_description(name: str, type_flag: bool) -> str:
    ...
    if type_flag:
        ...
        stm_index: int = int(
            re.search(r"(?<=STMS-)[0-9]{1,2}(?=-temp.tif)", name).group()
        )
...
def main() -> None:
    in_raster = cio.read_raster(args.get("source_dst"))
    in_array: np.ndarray = in_raster.GetRasterBand(1).ReadAsArray(
        0, 0, in_raster.RasterXSize, in_raster.RasterYSize
    )

    out_array: np.ndarray = scale_array(
        in_array, args.get("is_stm"), in_raster.GetRasterBand(1).GetNoDataValue()
    )
    out_description: str = generate_band_description(
        args.get("source_dst"), args.get("is_stm")
    )
    write_raster(
        args.get("output_dst"),
        out_array,
