...
def explode_multi_raster_to_vrt(multi_raster: gdal.Dataset, file_path: Path) -> List[Path]:
	"""
	...
	base_name: str = re.sub(r"\..*$", "", str(file_path.name))
	...
	for layer in range(1, n_bands + 1):
		...
		return_list.append(Path(vrt_out_name))
...
def create_big_cube(in_files: List[Path], out_name: str) -> None:
	if not (big_vrt := gdal.BuildVRT(out_name, [str(in_f) for in_f in in_files])):
